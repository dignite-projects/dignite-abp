...
public class TenantDomainAppService : ApplicationService, ITenantDomainAppService
{
    ...
    private readonly AbpMultiTenancyDomainOptions _options;
    private readonly IWebServerManager _webServerManager;
    private readonly IAuthServerRedirectUriManager _redirectUriManager;

    public TenantDomainAppService(ISettingManager settingManager, IOptions<AbpMultiTenancyDomainOptions> options, IWebServerManager webServerManager, IAuthServerRedirectUriManager authServerRedirectUriManager)
    {
    ...
    public async Task<TenantDomainDto> ConnectAsync(ConnectTenantDomainInput input)
    {
        ...
        if (isValid)
        {
            ...
            //将域名绑定到站点
            await _webServerManager.AddOrUpdateDomainAsync(input.DomainName, _options.UpstreamAddress, CurrentTenant.Id.Value, _options.WebServerSiteName);
            ...
            //
            return new TenantDomainDto(input.DomainName, true, CurrentTenant.Id);
        }
        else
        {
            return new TenantDomainDto(input.DomainName, false, CurrentTenant.Id);
        }
    }
    ...
    public async Task<TenantDomainDto?> GetAsync()
    {
        ...
        return new TenantDomainDto(domainName, isValid, CurrentTenant.Id);
    }
